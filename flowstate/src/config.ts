...
import { AppConfig, UserPreferences, APIConfig } from './types.js';
...
export class ConfigManager {
  ...
  async updateAPIConfig(updates: Partial<APIConfig>): Promise<void> {
    const config = await this.load();
    config.apiConfig = {
      ...config.apiConfig,
      ...updates,
  ...
  async setAPIKey(service: keyof APIConfig, apiKey: string): Promise<void> {
    const config = await this.load();
    if (!config.apiConfig[service]) {
      throw new Error(`Unknown API service: ${service}`);
    }
    
    (config.apiConfig[service] as any).apiKey = apiKey;
    await this.save();
  }

  async getAPIKey(service: keyof APIConfig): Promise<string | undefined> {
    const config = await this.load();
    return (config.apiConfig[service] as any)?.apiKey;
  }
