...
program
  ...
  .action(async (taskId: string, opts: { duration?: string, withDetection?: boolean }) => {
    const t = await store.getTask(taskId);
    if (!t) { console.error('Task not found'); process.exit(1); }
...
program
  ...
  .action(async (service: string, key: string) => {
    try {
      await config.setAPIKey(service as any, key);
...
program
  ...
  .action(async (opts: { set?: string[], list?: boolean }) => {
    if (opts.list) {
      const cfg = await config.load();
...
program
  ...
  .action(async (taskId: string, opts: { interval: string }) => {
    const t = await store.getTask(taskId);
    if (!t) { console.error('Task not found'); process.exit(1); }
